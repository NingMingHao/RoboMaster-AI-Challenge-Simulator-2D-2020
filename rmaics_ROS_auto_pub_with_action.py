...
from std_msgs.msg import Int16, Int32 ### recieving start signal
import rospy
from roslaunch.parent import ROSLaunchParent
import tf
import actionlib
from roborts_msgs.msg import ArmorDetectionAction, ArmorDetectionFeedback
import threading
from kernal import kernal
import numpy as np


class ArmorDetectionNode(object):
    detected = False
    running_ = True
    patrol_dir = 1
    patrol_count = 0
    patrol_mode = 0
    mode_angle = np.array( [ [70, 45, 20, -20, -45, -70],
                             [70, 45, 20, -20],
                             [20, -20],
                             [20, -20, -45, -70],
                             [70, 45, -45, -70],
    ...
    def __init__(self, game, ns):
        self.node_state_ = 'IDLE'
        self.thread_ = None
        self.game = game
        self.ns = ns
        self._server_name = self.ns + '/armor_detection_node_action'
        self._as = actionlib.SimpleActionServer(self._server_name, ArmorDetectionAction, execute_cb=self.execute_cb, auto_start = False)
        self._as.start()
        self.patrol_suggest_sub_ = rospy.Subscriber(self.ns+'/patrol_suggest', Int32, callback=self.PatrolSuggestCallback, queue_size=1)
        
    def execute_cb(self, goal):
        feedback = ArmorDetectionFeedback()
        ...
        while not rospy.is_shutdown():
            if(self._as.is_preempt_requested()):
                ...
                return
            enemy_info_msg = self.get_enemy_info()
            if len(enemy_info_msg):
                self.detected = True
                feedback.detected = True
                feedback.enemy_info = enemy_info_msg
                self._as.publish_feedback(feedback)
                undetected_msg_published = False
            else:
                self.detected = False
                if not undetected_msg_published:
    ...
    def get_enemy_info(self):
        ...
        vision = self.game.vision
        cars = self.game.agents
        who_is_enemy = {0:[1,3], 1:[0,2], 2:[1,3], 3:[0,2]}
        i = {'blue1':0, 'red1':1, 'blue2':2, 'red2':3}[self.ns]
        ...
        for enemy_ind in who_is_enemy[i]:
            if tmp_vision[enemy_ind]:
                ...
                enemy_info_msg.append(one_enemy_info)
        return enemy_info_msg
    
        
    def GetPatrolAngle(self):
        if self.patrol_count >= len(self.mode_angle[self.patrol_mode])-1:
            self.patrol_count = len(self.mode_angle[self.patrol_mode])-1
            self.patrol_dir = -1
        if self.patrol_count <= 0:
            self.patrol_count = 0
            self.patrol_dir = 1
        patrol_angle = self.mode_angle[self.patrol_mode][self.patrol_count]
        self.patrol_count += self.patrol_dir;
        return patrol_angle
    ...
    def ExecuteLoop(self):
        ...
        while self.running_:
            time.sleep(0.4)
            if self.node_state_ == 'RUNNING':
                partol_angle = self.GetPatrolAngle()
                if not self.detected:
                    self.game.cars[i, 4] = partol_angle
            
        
    def StartThread(self):
        self.running_ = True
        if self.node_state_ == 'IDLE':
            self.thread_ = threading.Thread(target=self.ExecuteLoop)
            self.thread_.start()
        self.node_state_ = 'RUNNING'
        
    
    def PauseThread(self):
        self.node_state_ = 'PAUSE'
    ...
    def PatrolSuggestCallback(self, mode):
        if mode.data <= 6:
            self.patrol_mode = mode.data
...
class rmaics(object):
    ...
    def publish_every_thing(self):
        ...
        game_time, cars, buff_info, done, detect, vision = self.state.time, self.state.agents, self.state.buff, self.state.done, self.state.detect, self.state.vision
        ...
        game_state_msg.remaining_time = game_time
...
#            self.br.sendTransform(tmp_translation, tmp_rotation, rospy.Time.now(), self.ns_names[i]+'/base_link', self.ns_names[i]+'/odom')
    ...
    def play_using_ros(self, save_file_name='./records/record1.npy'):
        ...
        self.action_nodes = [ArmorDetectionNode(self.game, ns) for ns in self.ns_names]
        self.launch_cars()
        rospy.spin()
